...
void Rc_Fun(T_RC_Data *rc_in,T_RC_Control *rc_ct)
{
	...
	if(rc_in->THROTTLE<RC_FUN_MIN&&rc_in->YAW<RC_FUN_MIN)  //用于解锁飞控
	{																											 
		cnt_arm++;
		if(cnt_arm==75)
		{
	...
	else if(rc_in->THROTTLE<RC_FUN_MIN&&rc_in->YAW>RC_FUN_MAX)  //用于给飞控上锁
	{
		cnt_arm++;
		if(cnt_arm==75)
		{
	...
	if(rc_in->THROTTLE<RC_FUN_MIN&&rc_in->ROLL<RC_FUN_MIN)   //RC_FUN_MIN=1200
	{
		...
		{
			cnt_fun = 0;
			MPU6050_CalOff_Acc();   //ACC 校准零偏标志为0
		}
	}
	else if(rc_in->THROTTLE<RC_FUN_MIN&&rc_in->ROLL>RC_FUN_MAX) //RC_FUN_MAX=1800
	{
		...
		{
			cnt_fun = 0;
			MPU6050_CalOff_Gyr();  //Gyr 校准零偏标志为0
		}
	}
	else
		cnt_fun = 0;
